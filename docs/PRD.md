...
> **版本**: v3.2 - 迭代 6 工作台管理报表重构
> **日期**: 2026-06-02
> **说明**: 本文档基于现有代码逆向梳理，含迭代 2 新增功能（FR-013~FR-015）、迭代 3 RBAC 权限拦截（FR-016）、迭代 4 Dashboard 前端集成、迭代 5 财务管理模块（FR-017）、迭代 6 工作台管理报表重构（FR-018）。
...
| 17 | 财务管理 | 应收账款管理、收款登记与核销、退款管理、收支明细、开票管理、财务对账 | 待开发 | [FR-017](prd/FR-017-finance-management.md) |
| 18 | 工作台重构 | 经营健康度、现金流与应收、团队效能、风险预警、决策支撑（管理层驾驶舱） | 待开发 | [FR-018](prd/FR-018-dashboard-redesign.md) |
...
**计划重构（v3.2，详见 [FR-018](prd/FR-018-dashboard-redesign.md)）**：

管理层反馈当前工作台过于操作层，缺乏决策支撑价值。v3.2 将重构为"管理层驾驶舱"，提供五大维度的数据洞察：

| 模块 | 核心指标 | 决策价值 |
|------|----------|----------|
| 经营健康度 | 营收/订单/客单价/签约率/毛利（含趋势、目标达成） | 一眼判断经营状况 |
| 现金流与应收 | 现金流入/应收余额/逾期应收/周转天数/账龄分析 | 把握资金健康 |
| 团队效能 | 人均产出/转化漏斗/销售排行榜 | 评估团队战斗力 |
| 风险预警 | 逾期应收/即将到期活动/长期未跟进/目标告急 | 主动发现风险 |
| 决策支撑 | 客户来源ROI/服务类型占比/供应商性价比 | 支撑资源分配决策 |

**API 变更**：
- 新增 6 个 v1 API（见 FR-018 §5）
- 旧 API 保持兼容（标记 deprecated，6 个月后移除）
